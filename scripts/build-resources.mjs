...
// drive the triefact-generation status display. Same uv-delegation trick as
// trie-mcp: the project venv is always used and the uv path survives
// Electron's stripped PATH.
// ---------------------------------------------------------------------------
...
# Runs the trie CLI via uv so the project venv is always used.
# uv path is baked in at build time to survive Electron's stripped PATH.
# The working directory (project root) is set by the caller, not baked in,
# because the CLI resolves config + .trie/ from cwd.
TRIE_REPO="${repoRoot}"
exec "${uvBin}" run --project "$TRIE_REPO" trie "$@"
`
writeFileSync(cliWrapperPath, cliWrapperContent, { mode: 0o755 })
console.log(`  ✓ resources/trie-cli (shell wrapper → ${uvBin})`)
